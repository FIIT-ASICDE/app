...
import { GithubRepoDisplay, RepositoryVisibility } from "@/lib/types/repository";
...
export const ImportRepositoryDialog = () => {
    ...
    const handleImportRepository = async () => {
        ...
        router.push("/" + newImportRepo.ownerName + "/" + newImportRepo.repoName);
    };
    ...
    const getOrganisationName = () => {
        const organisation = usersOrganisations.data?.find((organisation) => organisation.id === selectedOwner);
                ...
                <ScrollArea className="h-full max-h-[90vh]">
                    <div className="p-6">
                        ...
                        <div className="space-y-4 pt-4">
                                ...
                                <Select
                                    ...
                                    onValueChange={(value: string) => {
                                        setSelectedRepository(
                                            githubOwnerRepositories.data?.repos?.find((ownerRepo) => ownerRepo.name === value) ??
                                            githubCollaboratorRepositories.data?.repos?.find((contributorRepo) => contributorRepo.name === value) ??
                                            githubMemberRepositories.data?.repos?.find((memberRepo) => memberRepo.name === value)
                                        );
                                    }}
                                >
                                    <SelectTrigger>
                                    ...
                                    <SelectContent className="max-h-[200px] max-w-[375px]">
                                        <SelectGroup>
                                            <SelectLabel className="text-muted-foreground">Personal
                                                repositories</SelectLabel>
                                            {githubOwnerRepositories.data?.repos?.map((ownerRepo: GithubRepoDisplay, index: number) => (
                                                <SelectRepositoryItem
                                                    key={index + ownerRepo.githubFullName}
                                        ...
                                        <SelectGroup>
                                            <SelectLabel className="text-muted-foreground">Collaborator
                                                repositories</SelectLabel>
                                            {githubCollaboratorRepositories.data?.repos?.map((contributorRepo: GithubRepoDisplay, index: number) => (
                                                <SelectRepositoryItem
                                                    key={index + contributorRepo.githubFullName}
                                                    repository={contributorRepo}
                                                />
                                            ))}
                                        </SelectGroup>

                                        <SelectGroup>
                                            <SelectLabel className="text-muted-foreground">Organisation
                                                repositories</SelectLabel>
                                            {githubMemberRepositories.data?.repos?.map((memberRepo: GithubRepoDisplay, index: number) => (
                                                <SelectRepositoryItem
                                                    key={index + memberRepo.githubFullName}
                            ...
                            <div className="space-y-2">
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        ...
                                        {usersOrganisations.data?.length !== 0 && (
                                            <SelectGroup>
                                                <SelectLabel className="text-muted-foreground">Your
                                                    organisations</SelectLabel>
                                                {usersOrganisations.data?.map((organisation) => (
                                                        <SelectItem
                                                            key={organisation.id}
                                ...
                                >
                                    ...
                                    <SelectContent className="max-h-[200px] max-w-[375px]">
                                        <SelectGroup>
                                            ...
                                            {branches.data && branches.data.map((branch: string, index: number) => (
                                                <SelectItem
                                                    key={index + branch}
                                                    value={branch}
                                                    className="cursor-pointer hover:bg-accent"
                                                >
                                                    <div
                                                        className="flex flex-row items-center gap-x-2 min-w-0 w-[315px]">
                                ...
                                >
                                    <div
                                        className={cn(
                                            ...
                                            selectedVisibility === "public" ? "border-accent" : "border-transparent",
                                        )}
                                        onClick={() => setSelectedVisibility("public")}
                                    >
                                        <RadioGroupItem value="public" />
                                        <Label
                                            className="flex w-full cursor-pointer flex-row items-center gap-x-3 font-normal">
                                            <Globe className="h-5 w-5" />
                                            <div className="space-y-1">
                                                <p className="text-sm font-medium leading-none">Public</p>
                                                <p className="text-sm text-muted-foreground">Anyone can see this
                                                    repository.</p>
                                            </div>
                                        </Label>
                                    </div>
                                    <div
                                        className={cn(
                                            "flex cursor-pointer flex-row items-center space-x-3 space-y-0 rounded border p-3 hover:bg-accent",
                                            selectedVisibility === "private" ? "border-accent" : "border-transparent",
                                        )}
                            ...
                            {selectedRepository && selectedBranch && selectedOwner && selectedVisibility && (
                                <DialogDescription className="text-muted-foreground">
                                    You are about to import a {selectedVisibility} repository called
                                    <span className="font-bold"> {selectedRepository.name} </span>
                                    and its
                                    ...
                                    {selectedOwner === user.id ? (
                                        ...
                                        <span> the {getOrganisationName()} organisation.</span>
                                    )}
                                </DialogDescription>
                            )}
                        </div>
